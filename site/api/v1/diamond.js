const {model,runAction}=require('../../lib/diamond-model');
...
module.exports=(req,res)=>{
  try{
    if(req.method==='GET')return send(res,200,{ok:true,...model(),actions:['model','normalize_event','pivot','contextualize','thread','group','group_family','activity_attack']});
    if(req.method!=='POST')return send(res,405,{ok:false,error:'GET or POST required'});
    return send(res,200,{ok:true,...runAction(body(req))});

...
const d3fend=require('../site/lib/d3fend-model.js');
const dml=require('../site/lib/dml-model.js');
...
function checkRegistry(reg){
  ...
  if(reg?.suite?.analysis_models?.defense?.contract!=='shatteredcastles.d3fend.model.v1')fail('D3FEND defense-model contract missing from suite registry');
  if(reg?.suite?.analysis_models?.defense?.endpoint!=='/api/v1/d3fend')fail('D3FEND endpoint missing from suite registry');
  if(reg?.suite?.analysis_models?.maturity?.contract!=='shatteredcastles.dml.model.v1')fail('DML maturity-model contract missing from suite registry');
  if(reg?.suite?.analysis_models?.maturity?.endpoint!=='/api/v1/dml')fail('DML endpoint missing from suite registry');
...
function checkSourceD3fend(){const m=d3fend.model();if(m.contract!=='shatteredcastles.d3fend.model.v1'||m.version!=='1.6.0')fail('source D3FEND contract/version mismatch');if(m.counts?.defensive_tactics!==7||m.counts?.defensive_techniques!==271||m.counts?.top_level_technique_families!==30)fail('source D3FEND counts mismatch');if(m.source?.ontology_ttl?.sha256!=='4909a5bb66b75d2c359624398848936fb56a6b246bcd5cfcd277977a1277753a')fail('source D3FEND TTL hash mismatch');if(m.semantics?.full_mappings_are_inferred_relationships!==true||m.semantics?.automatic_remediation!==false)fail('source D3FEND semantics unsafe');return{contract:m.contract,version:m.version,counts:m.counts,mapping_attack_version:m.source.mapping_attack_version,digest:m.model_digest_sha256,catalog_digest:m.catalog_digest_sha256};}
function checkSourceDml(){const m=dml.model();if(m.contract!=='shatteredcastles.dml.model.v1'||m.version!=='Stillions-2014-original')fail('source DML contract/version mismatch');if(m.levels?.length!==9||m.levels?.[0]?.level!==0||m.levels?.[8]?.level!==8)fail('source DML levels mismatch');if(m.semantics?.detection_not_prevention!==true||m.semantics?.one_detection_does_not_establish_organization_wide_maturity!==true)fail('source DML semantics unsafe');return{contract:m.contract,version:m.version,levels:m.levels.length,digest:m.model_digest_sha256};}
...
async function checkPublicD3fend(reg){const url=new URL(reg.suite.analysis_models.defense.endpoint,'https://glasscastles.vercel.app');const r=await fetch(url,{headers:{accept:'application/json'}});if(!r.ok)fail(`public D3FEND HTTP ${r.status}`);const m=await r.json();if(m.contract!=='shatteredcastles.d3fend.model.v1'||m.version!=='1.6.0'||m.counts?.defensive_techniques!==271)fail('public D3FEND model mismatch');if(m.semantics?.full_mappings_are_inferred_relationships!==true||m.semantics?.automatic_remediation!==false)fail('public D3FEND semantics unsafe');return{url:String(url),contract:m.contract,version:m.version,counts:m.counts,mapping_attack_version:m.source?.mapping_attack_version,digest:m.model_digest_sha256,catalog_digest:m.catalog_digest_sha256};}
async function checkPublicDml(reg){const url=new URL(reg.suite.analysis_models.maturity.endpoint,'https://glasscastles.vercel.app');const r=await fetch(url,{headers:{accept:'application/json'}});if(!r.ok)fail(`public DML HTTP ${r.status}`);const m=await r.json();if(m.contract!=='shatteredcastles.dml.model.v1'||m.version!=='Stillions-2014-original'||m.levels?.length!==9)fail('public DML model mismatch');if(m.semantics?.detection_not_prevention!==true||m.semantics?.one_detection_does_not_establish_organization_wide_maturity!==true)fail('public DML semantics unsafe');return{url:String(url),contract:m.contract,version:m.version,levels:m.levels.length,digest:m.model_digest_sha256};}
...
async function checkCapabilities(reg){
  ...
  for(const [stage,contract] of EXPECTED){
    ...
    if(body?.suite?.d3fend?.contract!=='shatteredcastles.d3fend.model.v1')fail(`${stage} D3FEND suite contract missing`);
    if(body?.suite?.dml?.contract!=='shatteredcastles.dml.model.v1')fail(`${stage} DML suite contract missing`);
...
if(mode==='source'||mode==='all')out.source={workflow:(await loadSourceRegistry()).workflow,ukc:checkSourceUkc(),diamond:checkSourceDiamond(),pyramid:checkSourcePyramid(),attack:checkSourceAttack(),d3fend:checkSourceD3fend(),dml:checkSourceDml(),ok:true};
if(mode==='public'||mode==='all'){
  const reg=await loadPublicRegistry();out.public={workflow:reg.workflow,capabilities:await checkCapabilities(reg),ukc:await checkPublicUkc(reg),diamond:await checkPublicDiamond(reg),pyramid:await checkPublicPyramid(reg),attack:await checkPublicAttack(reg),d3fend:await checkPublicD3fend(reg),dml:await checkPublicDml(reg),ok:true};
}

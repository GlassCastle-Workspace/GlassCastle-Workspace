...
if(mode==='source'||mode==='all')out.source={workflow:(await loadSourceRegistry()).workflow,ukc:checkSourceUkc(),diamond:checkSourceDiamond(),pyramid:checkSourcePyramid(),attack:checkSourceAttack(),d3fend:checkSourceD3fend(),ok:true};
if(mode==='public'||mode==='all'){
  const reg=await loadPublicRegistry();out.public={workflow:reg.workflow,capabilities:await checkCapabilities(reg),ukc:await checkPublicUkc(reg),diamond:await checkPublicDiamond(reg),pyramid:await checkPublicPyramid(reg),attack:await checkPublicAttack(reg),d3fend:await checkPublicD3fend(reg),ok:true};
}
